        ...
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-aqua">
            <div class="inner">
              ...
              {
                 $stmt = $conn->prepare("SELECT *  FROM details LEFT join products on details.product_id=products.id LEFT JOIN sales ON details.sales_id=sales.id where products.user_id=:user_id and o_status !='Canceled'");
                ...
                echo "<h3>".number_format_short($total, 2)." EGP</h3>";
              ?>
              <p>Total Sales</p>
            </div>
            <div class="icon">
        ...
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-yellow">
            <div class="inner">
              ...
              {
                $stmt = $conn->prepare("SELECT COUNT(*) AS numrows FROM details LEFT join products on details.product_id=products.id LEFT JOIN sales ON details.sales_id=sales.id where products.user_id=:user_id and o_status !='Canceled'");
                $stmt->execute(['user_id'=>$admin['id']]);
              }
                // $stmt = $conn->prepare("SELECT *, COUNT(*) AS numrows FROM users");
          ...
          <div class="small-box bg-red">
            <div class="inner">
              ...
              if ($admin['type']==1) {
                ...
                $stmt = $conn->prepare("SELECT * FROM details LEFT JOIN sales ON sales.id=details.sales_id LEFT JOIN products ON products.id=details.product_id WHERE sales_date=:sales_date and products.user_id=:user_id and sales.o_status !='Canceled'");
                $stmt->execute(['sales_date'=>$today,'user_id'=>$admin['id']]);
                }
                $total = 0;
...
<?php
  ...
  for( $m = 1; $m <= 12; $m++ ) {
    try{
      ...
      {
        $stmt = $conn->prepare("SELECT * FROM details LEFT JOIN sales ON sales.id=details.sales_id LEFT JOIN products ON products.id=details.product_id WHERE MONTH(sales_date)=:month AND YEAR(sales_date)=:year and products.user_id=:user_id and sales.o_status !='Canceled'");
